...
LANG_CONFIG = {
    ...
    'vi': {
        'lang_attr': 'vi',
        'font_family': "'Noto Serif', 'DejaVu Serif', Georgia, serif",
        'font_family_ebook': '"Noto Serif", "DejaVu Serif", Georgia, serif',
        'toc_label': 'Mục lục',
        'pdf_font': 'Noto Serif',
    },
    'pt': {
...
_DEFAULT_LANG_CONFIG = {
    'lang_attr': 'en',
    'font_family': "'Noto Serif', Georgia, 'Times New Roman', Times, serif",
    'font_family_ebook': '"Noto Serif", Georgia, "Times New Roman", Times, serif',
    'toc_label': 'Contents',
    'pdf_font': 'Noto Serif',
}
...
def generate_pdf_weasyprint(temp_dir):
    """Generate a print-quality PDF using WeasyPrint (preferred over Calibre for PDF).

    Reads book_doc.html, wraps it in template_print.html, and renders with WeasyPrint.
    Returns the output path on success, None on failure or if WeasyPrint is unavailable.
    """
    try:
        import weasyprint  # noqa: F401
    except ImportError:
        return None  # WeasyPrint not installed — caller falls back to Calibre
    ...
    weasyprint_script = os.path.join(SCRIPT_DIR, 'weasyprint_pdf.py')
    if not os.path.exists(weasyprint_script):
        print("WARNING: weasyprint_pdf.py not found — falling back to Calibre for PDF")
        return None

    # Skip regeneration when the PDF is already up-to-date
    if os.path.exists(output_file):
        output_mtime = os.path.getmtime(output_file)
        html_newer = os.path.getmtime(html_file) > output_mtime
        images_newer = False
        ...
        if not html_newer and not images_newer:
            file_size = os.path.getsize(output_file)
            print(f"Skipping PDF (WeasyPrint) — already up to date ({file_size:,} bytes)")
            return output_file

    print("Generating PDF with WeasyPrint (print-quality template)…")
    try:
        result = subprocess.run(
            ['python3', weasyprint_script, html_file, '-o', output_file],
            capture_output=True, text=True, timeout=300
        )
        if result.returncode == 0 and os.path.exists(output_file):
            file_size = os.path.getsize(output_file)
            print(f"  .pdf: OK ({file_size:,} bytes) — WeasyPrint")
            return output_file
        else:
            print(f"WeasyPrint PDF failed (exit {result.returncode}); falling back to Calibre")
            if result.stderr:
                print(f"  stderr: {result.stderr[-400:]}")
            return None
    except subprocess.TimeoutExpired:
        print("WeasyPrint PDF timed out; falling back to Calibre")
        return None
    except Exception as e:
        print(f"WeasyPrint PDF error: {e}; falling back to Calibre")
        return None


def generate_formats(temp_dir, lang_attr, cover=None, pdf_only=False):
    """Generate output formats. With pdf_only=True, skip DOCX/EPUB."""
    ...
    # PDF: try WeasyPrint first (better typography), fall back to Calibre
    wp_result = generate_pdf_weasyprint(temp_dir)
    if wp_result:
        file_size = os.path.getsize(wp_result)
        results['.pdf'] = ('OK', f"{file_size:,} bytes (WeasyPrint)")
    else:
        result = generate_format(html_file, temp_dir, '.pdf', lang_attr, cover=cover)
        if result:
            file_size = os.path.getsize(result)
            results['.pdf'] = ('OK', f"{file_size:,} bytes (Calibre)")
        else:
            results['.pdf'] = ('FAILED', '')
...
def main():
    ...
    all_formats_ok = generate_formats(temp_dir, lang_cfg['lang_attr'], cover=cover, pdf_only=args.pdf_only)
