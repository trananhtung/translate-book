...
def convert_html_to_markdown(html_file, md_file, strip_page_numbers=False):
    ...
    try:
        # Call pandoc directly via subprocess to avoid pypandoc's glob expansion
        # which breaks on paths containing [ ] characters.
        import shutil as _shutil
        pandoc_bin = _shutil.which('pandoc')
        if not pandoc_bin:
            print("pandoc not found. Install pandoc to enable HTML→Markdown conversion.")
            return False
        result = subprocess.run(
            [pandoc_bin, '--from=html', '--to=markdown', '--wrap=none',
             '--output', str(md_file), str(html_file)],
